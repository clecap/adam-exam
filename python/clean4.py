...
def _dedup_boxes(decoded) -> list[tuple[int, int, int, int]]:
  """
  Deduplicate QR detections by bounding boxes.
  ...
  for sym in decoded:
    r = sym.rect
    boxes.add((int(r.left), int(r.top), int(r.width), int(r.height)))
  return sorted(boxes)
...
def decode_qr_with_variants(img_rgb: Image.Image) -> dict:
  ...
  def consider(im: Image.Image) -> None:
    nonlocal best_count, best_boxes, best_h, best_w
  ...
  return {"count": best_count, "boxes": best_boxes, "img_h": best_h, "img_w": best_w}
...
def count_qr_codes_worker(pdf_path: str, page_index: int, dpi: int) -> dict:
  ...
  try:
    ...
    return {"ok": True, "count": count, "rotate180": rotate180, "error": None}
  except Exception:
    # Any failure -> main process treats page as buggy.
    return {"ok": False, "count": 0, "rotate180": False, "error": traceback.format_exc()}
...
def main() -> int:
  ...
  p.add_argument(
    ...
    dest="report_out",
    default=None,
    help="Path for final report text file (default: <input_dir>/qr_split_report.txt)"
  )
